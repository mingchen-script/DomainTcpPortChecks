# Domain TCP ports checks v3:
#   Quick TCP port tests for TrustCreate opreation. Feel free to modify to fit your needs.
    ...
    PROCESS
    {
        ...
        If ($EPMOpen)
        {
            ...
            If ($mcRPCResult -eq $True) {
                ...
                Write-Output ">>>>>>> RPC ports Unreachable  <<-=-=-=-=<<<<`n    None of these RPC ports are Reachable:`n      [$PortDeDup]`n    Please open TCP 49152-65535 RPC ports for LSA, SAM, NetLogon (*) per:`n    https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/config-firewall-for-ad-domains-and-trusts`n`n"
            }
            #--------Mod end-----------------
        }
    }
